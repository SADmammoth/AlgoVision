async function quickSort(array, 
  ...
  onGroupChange=(array,left,right)) {
    ...
    async function sort(arr, left, right) {
      ...
      do {
        while (arr[i].valueOf() < mid.valueOf()) {await onСompare(arr, arr[i], mid); onMidChange(arr,mid);     onGroupChange(arr, left,right);i++};
        while (arr[j].valueOf() > mid.valueOf()) {await onСompare(arr, arr[j], mid);  onMidChange(arr,mid);     onGroupChange(arr, left,right);j--};
